...
const signIn = async(userMeta) => {
    ...
    createUser = await dbService.updateOneService('User', checkExistParams, addUser);
    if (!createUser._id) {
...
const logout = async(params) => {
    ...
    const user = await dbService.checkExists('User', updateCondtion);
    if (!user || !user._id) {
        throw boom.notFound('User not Found.');
    }
    ...
    const updateUser = await dbService.updateOneService('User', updateCondtion, updateParams);
    const result = {
        status: 200,
